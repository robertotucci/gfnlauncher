...
The lint above is expected to fail, with **five** errors — `finish-args-flatpak-spawn-access`, the three `com.nvidia.geforcenow` folder permissions, and `finish-args-kwin-talk-name`. They are not defects to fix; they are what the launcher is, and Flathub grants them as exceptions during review. The pull request has to argue each one, and the arguments are the comments beside the permissions in the manifest, which is why the swap above preserves them. The KWin one is the easiest to argue and the easiest to forget to: it is a read and a `forceActivate` against the session's own on-screen keyboard, it is one branch of a per-desktop question rather than the only path, and revoking it costs a keyboard the launcher draws itself anyway.

`--system-talk-name=org.freedesktop.locale1` draws no lint error and should still be mentioned: it reads `X11Layout` and nothing else, localed needs polkit to *change* a layout and nothing to report one, and without it a keyboard is drawn in the wrong layout on every desktop that is not KDE or GNOME.
