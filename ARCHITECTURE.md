...
- **It must not call `stepAside`.** That stream is our own window; a minimised launcher behind it could not be raised again once it closed, because the pad cannot focus a blurred window. `restoreLauncher` in `main/window.ts` is the way back, and it re-applies fullscreen for the same reason `second-instance` does.

### Getting the screen back

**The client does not exit when a stream ends.** It returns to its own mall, fullscreen, holding the focus — and on a machine with no mouse that holds the launcher hostage, because the Gamepad API only reports to a focused window. Two changes follow, and they are a pair:
