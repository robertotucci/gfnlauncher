...
**There are two renderer pages.** `src/renderer/index.html` is the launcher; `src/renderer/compose.html` is the keyboard that types into windows the launcher does not own — no React, no Tailwind, no state, because main drives it over the bridge. Both are named in `rollupOptions.input`, and a page that is not named there is a page that silently does not ship.
...
- **LB + RB with the desktop cursor up, over something you can type into.** A keyboard appears **flush with the bottom of the screen**, in your session's own layout — switch the system layout and reopen it to see it follow — and every character on it must actually type, the accented letters included. Write a word, press SEND, and it must land in the field behind — including a text box inside a *streamed game*, which is the case the whole feature exists for. Then do it again and cancel with ☰: nothing may be typed. Leave pointer mode with the keyboard still up and confirm the window goes with it.
  ...
  - **Every badge printed on a keycap does what it says.** X types a space, Y sends, B deletes one character, ☰ discards. On a Bluetooth Xbox pad X and Y are `BTN_WEST` and `BTN_NORTH`; the line each pad writes when it attaches is how to check where they landed.
  - **Settings → Appearance → Keyboard size, every step.** At the largest, with the **Italian** layout — which draws a row more than US — the panel must still be inside the screen. At the smallest the legends must still be readable from the sofa.
