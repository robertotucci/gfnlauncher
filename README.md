...
- **Fullscreen games, from the first frame** — GeForce NOW only takes the whole screen once a game is streaming; its menus and loading screen come up in a window with a title bar. The launcher asks the desktop to remove it, so nothing but the game is ever on screen. Through KWin on KDE, `xdotool` elsewhere, and if neither is there the game still launches
...
| `--talk-name=org.kde.KWin` | Two things, both of which only KDE can be asked for. Raising its own on-screen keyboard, and making the GeForce NOW window fullscreen and borderless when a game starts, so it does not come up with a title bar until the stream does. Absent everywhere else and never called there: the launcher resolves the desktop before it opens a bus, draws its own keyboard, and falls back to `xdotool` for the window. Revoke it on KDE and both still work that way, with a line in the log and the command to put it back |
